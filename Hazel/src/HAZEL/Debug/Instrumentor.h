...
namespace Hazel {
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json") {
			...
			if (mCurrentSession) {
				...
				if (Log::GetCoreLogger()) {	
					...
					HZ_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, mCurrentSession->name);
				} 
				InternalEndSession();
			}
			...
			if (mOutputStream.is_open()) {
				...
				if (Log::GetCoreLogger()) {
					// Edge case: BeginSession() might be before Log::Init()
					HZ_CORE_ERROR("Instrumentor cloud not open results file '{0}'.", filepath);
				} 
			}
		}
	...
	namespace InstrumentorUtils {
		...
		const auto CleanupOutputString(const char(&expr)[N], const char(&remove)[K]) {
...
#define HZ_PROFILE 0
	...
	// could mark the wrong one in your editor!
		...
		#define HZ_FUNC_SIG __PRETTY_FUNCTION__
	#elfi (defined(__FUNCSIG__) || (_MSC_VER))
